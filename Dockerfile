...
#
#   docker build --build-arg PHP_VERSION=8.4 --tag extractor-dev .
#   docker run --rm --volume "$PWD":/app extractor-dev composer install
#   docker run --rm --volume "$PWD":/app extractor-dev phpunit
#
...
# The test suite runs against an tests only talk to a stub HTTP server, so the official
# PHP images already provide. pdo_mysql is not used by the package, but Laravel's
# database configuration reads constants off it when it is there, so the CI
# runners have it and the test suite has to see the same deprecations they do.
RUN apt-get update \
